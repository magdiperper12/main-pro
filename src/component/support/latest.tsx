        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 md:grid-cols-2 gap-16 items-start w-10/12 md:w-9/12 m-auto'>
          {latestitems.map((item, index) => {
            ...
            return (
              <div
                key={index}
                className='hover:shadow-lg duration-200 text-center bg-zinc-100 p-2 rounded-lg min-h-80'
              >
                {typeof Icon === "string" ? (
                  <img
                    src={Icon}
                    alt={`${item.heading} icon`}
                    className='w-1/4 h-1/4 mx-auto my-4 hover:scale-105 duration-300 hover:drop-shadow-lg'
                  />
                ) : (
                  <Icon className='text-4xl md:text-6xl  text-blue-600 mx-auto my-4 hover:scale-105 duration-300 hover:drop-shadow-lg' />
                )}
